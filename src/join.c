...
static int build_relations(predicate *pred, uint32_t *relations, DArray *metadata_arr, DArray *mid_results, relation *rel[]) {
    ...
    if (relations[lhs_rel] == relations[rhs_rel]) {
        ...
        if (rhs_col == lhs_col) {
            ...
            error = allocate_relation_mid_results(rel, mid_results, metadata_arr, lhs_index, lhs_rel, rhs_col, 1);

            check(error != -1, "Couldn't allocate relations");

            return SCAN_JOIN;
        }
        else if (rhs_index != -1 && lhs_index == -1) {
            error = allocate_relation_mid_results(rel, mid_results, metadata_arr, rhs_index, rhs_rel, lhs_col, 0);
...
int execute_join(predicate *pred, uint32_t *relations, DArray *metadata_arr, DArray *mid_results) {
    ...
    if (retval == CLASSIC_JOIN) {
        ...
        join_result join_res = call_join(rel, &error);
    }
        ...
        join_result join_res = scan_join(rel[0], rel[1], &error);
    }
    else if (retval == -1) {
        return -1;
    }
